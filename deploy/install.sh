...
ensure_defaults(){ install -d -m 0750 "$ETC_DIR" "$DATA_DIR"; touch "$ENV_FILE"; chmod 0600 "$ENV_FILE"; local jwt master db host configured_port; configured_port="$(read_env PANEL_HTTP_PORT 2>/dev/null || true)"; if [[ -z "${PORT:-}" && -n "$configured_port" ]]; then PORT="$configured_port"; fi; jwt="$(read_env JWT_SECRET)"; [[ -n "$jwt" ]] || jwt="$(random_hex)"; master="$(read_env MASTER_SECRET)"; [[ -n "$master" ]] || master="$(random_hex)"; db="$(read_env DATABASE_URL)"; [[ "$db" == sqlite:* ]] || db="sqlite:////${DATA_DIR#/}/pars2ray.db"; host="$(hostname -I 2>/dev/null | awk '{print $1}')"; host="${host:-127.0.0.1}"; set_env ENVIRONMENT production; set_env DEBUG false; set_env DATABASE_URL "$db"; set_env REDIS_URL ""; set_env JWT_SECRET "$jwt"; set_env MASTER_SECRET "$master"; set_env PARS2RAY_DATA_DIR "$DATA_DIR"; if [[ -n "${PORT:-}" ]]; then set_env PANEL_HTTP_PORT "$PORT"; fi; set_env TRUSTED_HOSTS "localhost,127.0.0.1,$host"; }
