...
install_prerequisites(){
  export DEBIAN_FRONTEND=noninteractive; local need=0
  ...
  if (( need )); then
    ...
    apt-get -o Acquire::Retries=2 -o Acquire::http::Timeout=8 -o Acquire::https::Timeout=8 update -qq; apt-get install -y -qq ca-certificates curl git openssl
  fi
}
ensure_docker(){
  export DEBIAN_FRONTEND=noninteractive
  if ! command -v docker >/dev/null; then
    log "Installing Docker Engine"; apt-get -o Acquire::Retries=2 -o Acquire::http::Timeout=8 -o Acquire::https::Timeout=8 update -qq
    apt-get install -y -qq docker.io || curl -fsSL --connect-timeout 5 --max-time 20 https://get.docker.com | sh
  fi
...
checkout_project(){
  ...
  if [[ -d "$PARS2RAY_INSTALL_DIR/.git" ]]; then
    git -C "$PARS2RAY_INSTALL_DIR" diff --quiet || die "Existing checkout has uncommitted changes"; git -C "$PARS2RAY_INSTALL_DIR" diff --cached --quiet || die "Existing checkout has staged changes"
