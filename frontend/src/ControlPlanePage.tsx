...
import { api } from './api'
import type { ControlBalancer, ControlFallback, ControlOutbound } from './types'

const arrayOf = <T,>(value: unknown): T[] => Array.isArray(value) ? value as T[] : []
const itemsOf = <T,>(value: unknown): T[] => { if (Array.isArray(value)) return value as T[]; if (value && typeof value === 'object') { const raw = value as Record<string, unknown>; return arrayOf<T>(raw.items) } return [] }
const rulesOf = <T,>(value: unknown): T[] => { if (Array.isArray(value)) return value as T[]; if (value && typeof value === 'object') return arrayOf<T>((value as Record<string, unknown>).rules); return [] }

export function ControlPlanePage({ notify }: { notify: (message: string, kind?: 'success'|'error') => void }) {
  const [outbounds, setOutbounds] = useState<ControlOutbound[]>([])
  const [rules, setRules] = useState<Array<Record<string, unknown>>>([])
  const [balancers, setBalancers] = useState<ControlBalancer[]>([])
  const [fallbacks, setFallbacks] = useState<ControlFallback[]>([])
  ...
  async function load() {
    ...
    try {
      ...
      setOutbounds(itemsOf<ControlOutbound>(ob)); setRules(rulesOf<Record<string, unknown>>(rr)); setBalancers(itemsOf<ControlBalancer>(bb)); setFallbacks(itemsOf<ControlFallback>(ff))
  ...
  async function replaceList(kind: 'balancers'|'fallbacks') {
    try { if (kind === 'balancers') await api.replaceBalancers(balancers); else await api.replaceFallbacks(fallbacks); notify(`${kind} updated`); await load() }
  ...
  return <div className="stack">
    ...
    <section className="panel"><div className="panel-header"><div><span className="eyebrow">Traffic policy</span><h2>Routing rules</h2></div></div><div className="form-grid"><input className="input" placeholder="domain.example" value={domain} onChange={e => setDomain(e.target.value)}/><select className="input" value={routeOutbound} onChange={e => setRouteOutbound(e.target.value)}>{outbounds.map(item => <option key={String(item.id ?? item.tag)} value={String(item.tag ?? '')}>{String(item.tag ?? '')}</option>)}</select><button className="button primary" onClick={() => void addRule()}>Add rule</button></div><div className="list">{rules.map((item, index) => <div className="list-row" key={String(item.id ?? `${item.outboundTag ?? 'rule'}-${index}`)}><div><strong>{JSON.stringify(item.domain ?? item.ip ?? [])}</strong><small>→ {String(item.outboundTag ?? '')}</small></div><button className="button danger" onClick={() => void removeRule(String(item.id ?? ''))} disabled={!item.id}>Delete</button></div>)}</div></section>
    <section className="panel"><div className="panel-header"><div><span className="eyebrow">Traffic distribution</span><h2>Balancers</h2><p>{balancers.length} configured.</p></div><button className="button ghost" onClick={() => void replaceList('balancers')}>Save</button></div></section>
    <section className="panel"><div className="panel-header"><div><span className="eyebrow">Inbound recovery</span><h2>Fallbacks</h2><p>{fallbacks.length} configured.</p></div><button className="button ghost" onClick={() => void replaceList('fallbacks')}>Save</button></div></section>
  </div>
}
