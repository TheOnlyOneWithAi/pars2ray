...
export function ControlPlanePage({ notify }: { notify: (message: string, kind?: 'success'|'error') => void }) {
  ...
  async function load() {
    ...
    try {
      ...
      setOutbounds(ob.items); setRules(rr.rules); setBalancers(bb.items); setFallbacks(ff.items)
  ...
  return <div className="stack">
    ...
    <section className="panel"><div className="panel-header"><div><span className="eyebrow">Traffic policy</span><h2>Routing rules</h2></div></div><div className="form-grid"><input className="input" placeholder="domain.example" value={domain} onChange={e => setDomain(e.target.value)}/><select className="input" value={routeOutbound} onChange={e => setRouteOutbound(e.target.value)}>{outbounds.map(item => <option key={String(item.id ?? item.tag)} value={String(item.tag)}>{String(item.tag)}</option>)}</select><button className="button primary" onClick={() => void addRule()}>Add rule</button></div><div className="list">{rules.map(item => <div className="list-row" key={String(item.id)}><div><strong>{JSON.stringify(item.domain ?? item.ip ?? [])}</strong><small>→ {String(item.outboundTag ?? '')}</small></div><button className="button danger" onClick={() => void removeRule(String(item.id))}>Delete</button></div>)}</div></section>
