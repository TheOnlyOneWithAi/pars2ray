...
export function NodeProvisionPage({ t, locale, notify, nodes, reload }: Props) {
  ...
  async function testSSH() {
    ...
    try {
      await api.testSSH({ host: String(data.get('host')).trim(), username: String(data.get('username')).trim(), password: String(data.get('password') || '') })
      notify(t('completed'))
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api.provisionNode({ node_key: String(form.get('node_key')).trim().toUpperCase(), country: String(form.get('country')).trim().toUpperCase(), ssh: { host: String(form.get('host')).trim(), username: String(form.get('username')).trim(), password: String(form.get('password') || '') } })
  ...
  return <>
    ...
    {show && <Modal title={t('create')} onClose={() => setShow(false)} wide><form ref={formRef} className="modal-body form-grid" onSubmit={create}>
      ...
      <Field label="SSH PASSWORD"><input name="password" type="password" autoComplete="new-password" required/></Field>
