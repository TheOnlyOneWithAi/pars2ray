...
import { ProtocolConfigPage } from './ProtocolConfigPage'
import { XrayManagerPage } from './XrayManagerPage'
import { RulesPage } from './RulesPage'
import { BillingPage, DashboardPage, ExperimentsPage, OptimizerPage, RoutesPage, SubscriptionsPage, UsersPage } from './pages'
import type { Dashboard, Locale, Node, Page, Route, TelemetryPoint, TrafficBreakdown } from './types'
import { Icon, Spinner } from './ui'

const pages = ['dashboard','nodes','routes','protocols','xray','experiments','optimizer','ai-stats','users','subscriptions','billing','rules','settings'] as unknown as Page[]
...
export default function App() {
  ...
  if (pageName === 'dashboard') content = <DashboardPage {...common} dashboard={dashboard} nodes={nodes} routes={routes} telemetry={telemetry} trafficBreakdown={trafficBreakdown} openPage={setPage}/>
  else if (pageName === 'nodes') content = <NodeProvisionPage {...common} nodes={nodes} reload={loadAll}/>
  else if (pageName === 'routes') content = <RoutesPage {...common} routes={routes} nodes={nodes} reload={loadAll}/>
  else if (pageName === 'protocols') content = <ProtocolConfigPage routes={routes} notify={notify}/>
  else if (pageName === 'xray') content = <XrayManagerPage routes={routes} notify={notify}/>
  else if (pageName === 'experiments') content = <ExperimentsPage {...common}/>
  else if (pageName === 'optimizer') content = <OptimizerPage {...common} dashboard={dashboard}/>
  else if (pageName === 'ai-stats') content = <AiStatsPage {...common}/>
  else if (pageName === 'users') content = <UsersPage {...common}/>
  else if (pageName === 'subscriptions') content = <SubscriptionsPage {...common}/>
  else if (pageName === 'billing') content = <BillingPage {...common}/>
  else if (pageName === 'rules') content = <RulesPage {...common}/>
  else content = <AdminSettingsPage notify={notify}/>
  return <div className="app-shell"><aside className={`sidebar ${sidebar ? 'open' : ''}`}><div className="brand"><div className="logo-mark"><span>P</span></div><div><strong>Pars2Ray</strong><small>ENTERPRISE</small></div><button className="mobile-close" onClick={() => setSidebar(false)}><Icon name="close"/></button></div><nav>{pages.map(item => { const name = String(item); return <button key={name} className={pageName === name ? 'active' : ''} onClick={() => setPage(item)}><Icon name={name}/><span>{name === 'ai-stats' ? 'AI Stats' : name === 'xray' ? 'Xray Manager' : name === 'rules' ? 'Rules' : t(item as TranslationKey)}</span>{pageName === name && <i/>}</button> })}</nav><div className="sidebar-footer"><div><span className="live-dot"/><strong>{isMockMode() ? 'Demo mode' : t('masterOnline')}</strong></div><small>v2.2.0 · {isMockMode() ? 'Local test data' : t('production')}</small></div></aside>{sidebar && <button className="sidebar-scrim" aria-label="close" onClick={() => setSidebar(false)}/>}<main className="main"><header className="topbar"><div className="title-group"><button className="menu-button" onClick={() => setSidebar(true)}><Icon name="menu"/></button><div><span className="breadcrumb">PARS2RAY / {nameLabel(pageName, t).toUpperCase()}</span><h1>{nameLabel(pageName, t)}</h1></div></div><div className="top-actions"><span className={`mode-indicator ${isMockMode() ? 'demo' : dashboard.mode.toLowerCase()}`}><i/>{isMockMode() ? 'DEMO' : dashboard.mode}</span><button className="icon-btn refresh-button" title={t('refresh')} onClick={() => void loadAll().then(() => notify(t('completed')))}><Icon name="refresh"/></button><LocalePicker locale={locale} setLocale={setLocale}/><details className="account-menu"><summary><span className="avatar">SA</span></summary><div><span>Super Admin</span><small>{isMockMode() ? 'Local demo' : 'Control plane'}</small><button onClick={() => void api.logout().finally(() => location.reload())}>{t('signOut')}</button></div></details></div></header><div className="page-content">{content}</div></main><div className="toast-stack">{toasts.map(toast => <div className={`toast ${toast.kind}`} key={toast.id}><span>{toast.kind === 'success' ? <Icon name="check" size={16}/> : '!'}</span><p>{toast.message.replaceAll('_',' ')}</p><button onClick={() => setToasts(items => items.filter(item => item.id !== toast.id))}><Icon name="close" size={14}/></button></div>)}</div></div>
}
function nameLabel(page: string, t: (key: TranslationKey) => string) { return page === 'ai-stats' ? 'AI Stats' : page === 'xray' ? 'Xray Manager' : page === 'rules' ? 'Rules' : t(page as TranslationKey) }
