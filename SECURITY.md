...
Only normalized telemetry, route metadata, and bounded candidates are sent to the configured AI provider. Credentials and raw tunnel configuration are not included in the optimizer context. Requests set `store=false` and use strict structured outputs. If the provider is unavailable, the local policy retains the active route or searches bounded templates.
