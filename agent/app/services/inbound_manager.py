...
ALLOWED_XRAY = {"vless", "vmess", "trojan", "shadowsocks"}
...
def _public_inbounds(config: dict) -> list[dict]:
    ...
    for item in config.get("inbounds", []) if isinstance(config, dict) else []:
        ...
        rows.append({
            ...
            "protocol": item.get("protocol", item.get("type", "")),
...
def _apply_xray(item: dict, protocol: str, transport: str) -> None:
    ...
    if not isinstance(stream, dict):
        raise ValueError("invalid_xray_stream_settings")
    stream["network"] = transport
    if transport == "tcp":
        stream.pop("wsSettings", None)
        stream.pop("grpcSettings", None)
        stream.pop("httpupgradeSettings", None)
        stream.pop("xhttpSettings", None)
    elif transport == "websocket":
        stream.setdefault("wsSettings", {})
        stream.pop("grpcSettings", None)
        stream.pop("httpupgradeSettings", None)
        stream.pop("xhttpSettings", None)
    elif transport == "grpc":
        stream.setdefault("grpcSettings", {})
        stream.pop("wsSettings", None)
        stream.pop("httpupgradeSettings", None)
        stream.pop("xhttpSettings", None)
    elif transport == "httpupgrade":
        stream.setdefault("httpupgradeSettings", {})
        stream.pop("wsSettings", None)
        stream.pop("grpcSettings", None)
        stream.pop("xhttpSettings", None)
    elif transport == "xhttp":
        stream.setdefault("xhttpSettings", {})
        stream.pop("wsSettings", None)
        stream.pop("grpcSettings", None)
        stream.pop("httpupgradeSettings", None)
    elif transport == "quic":
...
def update_existing_inbounds(updates: list[dict]) -> dict:
    if not isinstance(updates, list) or not updates:
        return {"ok": False, "reason": "no_inbound_updates"}
    with _state_lock():
        if not ACTIVE.exists():
        ...
        if not isinstance(inbounds, list):
            return {"ok": False, "reason": "inbounds_missing"}
        ...
        if not allowed:
            return {"ok": False, "reason": "no_supported_core"}
        ...
        try:
            ...
            if active_core == "xray":
                ...
                command = ["sing-box", "check", "-c", str(tmp)]
            check = subprocess.run(command, capture_output=True, text=True, timeout=15, check=False)
            if check.returncode != 0:
                return {"ok": False, "reason": "config_validation_failed", "detail": (check.stderr or check.stdout)[-1000:]}
            _atomic_copy(ACTIVE, PREVIOUS)
            _atomic_copy(tmp, ACTIVE)
