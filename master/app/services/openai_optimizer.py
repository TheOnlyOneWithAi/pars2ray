...
SCHEMA = {"type": "object", "properties": {"action": {"type": "string", "enum": ["KEEP", "TEST", "CANARY", "SWITCH", "ROLLBACK"]}, "candidate_id": {"type": ["string", "null"]}, "confidence": {"type": "number", "minimum": 0, "maximum": 1}, "reason": {"type": "string", "maxLength": 500}}, "required": ["action", "candidate_id", "confidence", "reason"], "additionalProperties": False}
...
def safe_context(context: dict) -> dict:
    ...
    for candidate in context.get("candidates", [])[:20]:
        if not isinstance(candidate, dict):
            continue
        fields = ("candidate_id", "core", "protocol", "transport", "score", "latency_ms", "jitter_ms", "packet_loss_percent", "throughput_mbps", "stability_percent", "node_count")
        candidates.append({key: candidate[key] for key in fields if key in candidate})
...
def _runtime_config() -> tuple[bool, str, str]:
    db = SessionLocal()
    try:
        ...
        enabled = decrypt_secret(rows["ai.enabled"].value_enc).lower() == "true" if "ai.enabled" in rows else settings.ai_enabled
        model = decrypt_secret(rows["ai.model"].value_enc) if "ai.model" in rows else settings.openai_model
        api_key = decrypt_secret(rows["ai.api_key"].value_enc) if "ai.api_key" in rows else settings.openai_api_key
        return bool(enabled and api_key), model, api_key
    finally:
        db.close()
...
async def analyze(context: dict) -> tuple[dict, dict]:
    ...
    async with httpx.AsyncClient(timeout=settings.openai_request_timeout_seconds, headers={"Content-Type": "application/json", "User-Agent": "Pars2Ray-Optimizer/2.3"}) as client:
        response = await client.post(settings.openai_base_url.rstrip("/") + "/responses", headers={"Authorization": f"Bearer {api_key}"}, json=payload)
