...
XRAY_PROTOCOLS = {"vless", "vmess", "trojan", "shadowsocks", "http", "socks", "dokodemo-door", "wireguard", "tun"}
...
def _transport_settings(transport: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if transport == "kcp": return {"network": "kcp", "kcpSettings": {"mtu": int(cfg.get("mtu", 1350)), "tti": int(cfg.get("tti", 20)), "uplinkCapacity": int(cfg.get("uplink_capacity", 5)), "downlinkCapacity": int(cfg.get("downlink_capacity", 20)), "congestion": bool(cfg.get("congestion", False)), "readBufferSize": int(cfg.get("read_buffer", 2)), "writeBufferSize": int(cfg.get("write_buffer", 2)), "header": {"type": cfg.get("header_type", "none")}}}
...
def _common_xray(inbound: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if cfg.get("sniffing"):
        inbound["sniffing"] = {"enabled": True, "destOverride": cfg.get("sniff_dest_override", ["http", "tls", "quic"]), "metadataOnly": bool(cfg.get("sniff_metadata_only", False))}
    if cfg.get("mux"):
        inbound["mux"] = {"enabled": True, "concurrency": int(cfg.get("mux_concurrency", 8)), "xudpConcurrency": int(cfg.get("xudp_concurrency", 16)), "xudpProxyUDP443": cfg.get("xudp_proxy_udp443", "reject")}
    ...
    return {"log": {"loglevel": cfg.get("loglevel", "warning")}, "inbounds": [inbound], "outbounds": outbounds, "routing": deepcopy(cfg.get("routing") or {"domainStrategy": "AsIs"})}
...
def build_xray(route: dict[str, Any], clients: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if protocol in {"http", "socks", "dokodemo-door", "wireguard", "tun"} and transport != "tcp": raise ValueError("advanced_protocol_requires_tcp")
    inbound: dict[str, Any] = {"tag": route.get("tag", route.get("name", "pars2ray")), "port": int(cfg.get("port", 443)), "protocol": protocol, "settings": {}}
    if cfg.get("listen"): inbound["listen"] = cfg["listen"]
    if protocol in {"vless", "vmess"}:
        ...
        inbound["settings"] = {"accounts": [{"user": c.get("email") or c["id"], "pass": cfg.get("password") or c["id"]} for c in clients], "allowTransparent": bool(cfg.get("allow_transparent", False))}
    elif protocol == "socks":
        inbound["settings"] = {"auth": cfg.get("auth", "password" if clients else "noauth"), "accounts": [{"user": c.get("email") or c["id"], "pass": cfg.get("password") or c["id"]} for c in clients], "udp": bool(cfg.get("udp", True))}
    elif protocol == "dokodemo-door":
        inbound["settings"] = {"address": cfg.get("address", "127.0.0.1"), "port": int(cfg.get("target_port", 80)), "network": cfg.get("network", "tcp,udp"), "followRedirect": bool(cfg.get("follow_redirect", False))}
...
def build_singbox(route: dict[str, Any], clients: list[dict[str, Any]]) -> dict[str, Any]:
    protocol = str(route["protocol"]).lower(); transport = str(route.get("transport", "tcp")).lower(); cfg = deepcopy(route.get("config") or {})
