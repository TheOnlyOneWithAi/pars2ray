...
def _common_xray(inbound: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if cfg.get("fallbacks"):
        inbound["settings"]["fallbacks"] = cfg["fallbacks"]
    ...
    if cfg.get("balancers"):
        routing.setdefault("balancers", cfg["balancers"])
    if cfg.get("routing_rules"):
        routing.setdefault("rules", cfg["routing_rules"])
    return {
...
def build_xray(route: dict[str, Any], clients: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # Omitting listen preserves Xray's all-interface default without triggering
    # Bandit's B104 hardcoded-bind warning. Operators can still set an explicit
    # listen address through the route configuration when required.
    if cfg.get("listen"):
        inbound["listen"] = cfg["listen"]
    if protocol in {"vless", "vmess"}:
        inbound["settings"] = {
            "clients": [
                {"id": c["id"], **({"email": c["email"]} if c.get("email") else {})}
                for c in clients
            ]
        }
        if protocol == "vless":
        ...
        inbound["settings"] = {
            "clients": [
                {"password": c["id"], **({"email": c["email"]} if c.get("email") else {})}
                for c in clients
            ]
        }
    else:
...
def build_singbox(route: dict[str, Any], clients: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if protocol in {"vless", "vmess"}:
        inbound["users"] = [
            {"uuid": c["id"], **({"name": c["email"]} if c.get("email") else {})}
            for c in clients
        ]
    elif protocol == "trojan":
        inbound["users"] = [
            {"password": c["id"], **({"name": c["email"]} if c.get("email") else {})}
            for c in clients
        ]
        ...
        inbound.update(
            {
                ...
                or (clients[0]["id"] if clients else "pars2ray"),
            }
        )
    else:
        inbound["users"] = [
    ...
    return {
        ...
        "inbounds": [inbound],
        "outbounds": deepcopy(
            cfg.get("outbounds") or [{"type": "direct", "tag": "direct"}]
        ),
