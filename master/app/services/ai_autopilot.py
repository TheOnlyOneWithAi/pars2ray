...
from app.services.inbound_store import create_inbound, set_score
...
async def run(db: Session, user: User, dry_run: bool = False) -> dict:
    p = policy(db)
    if user.role != "SUPER_ADMIN" or p.level < 4 or not p.autonomous:
    ...
    for node in nodes:
        try:
            ...
            continue
    if not snapshots:
        return {"ok": True, "created": [], "tested": [], "probe_node": probe_node.node_key, "reason": "nodes_unreachable"}

    existing_ports: dict[str, set[int]] = {}
    from app.services.inbound_store import list_inbounds
