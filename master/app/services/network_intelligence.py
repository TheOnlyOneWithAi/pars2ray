...
from app.models.entities import Decision, Experiment, Route
...
def _safe_candidate(item: Experiment) -> dict:
    """Expose telemetry only; never include experiment metadata or route secrets."""
    return {
        ...
        "stability_percent": item.stability_percent,
        "node_count": len(item.node_keys or []),
    }


def build_intelligence_snapshot(route_id: int | None = None, hours: int = 24) -> dict:
    """Build bounded local evidence for the decision engine and optional AI advisor."""
    cutoff = datetime.utcnow() - timedelta(hours=max(1, min(hours, 168)))
    db = SessionLocal()
    try:
        route = (
            db.get(Route, route_id)
            if route_id
            else db.scalar(select(Route).where(Route.is_active.is_(True)).limit(1))
        )
        if not route:
            return {
                "current_route": {},
                ...
                "current_score": 0.0,
                "previous_score": 0.0,
                "trigger": "no_active_route",
            }

        experiments = db.scalars(
            select(Experiment)
            .where(Experiment.created_at >= cutoff)
        ...
        return {
            ...
            "current_score": route.score,
            "previous_score": previous.proposed_score if previous else route.score,
            "trigger": "scheduled_intelligence_cycle",
        }
    finally:
        db.close()
